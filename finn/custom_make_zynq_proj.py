...
class MakeZYNQProject(Transformation):
    ...
    def apply(self, model):
        ...
        config = "\n".join(config) + "\n"
        zynq_shell_template = templates.custom_zynq_shell_template.splitlines()[:-9]
        for i, l in enumerate(zynq_shell_template):
            if l.startswith("make_wrapper"):
                # Don't make wrapper
                zynq_shell_template.pop(i)
                break
        with open(
            f"{os.environ['RHD2164_SPI_FPGA_ROOT']}/vivado/insert_rhd2164.tcl",
            "r",
        ) as f:
            text = f.readlines()
            zynq_shell_template.extend(text)
        ...
        # run tcl recipe to make the BD and synthesize it
        bash_command = ["bash", synth_project_sh]
        process_compile = subprocess.Popen(bash_command, stdout=subprocess.PIPE)
        process_compile.communicate()

        # capture and copy outputs (.bit, .hwh)
        bitfile_name = (
        ...
        deploy_bitfile_name = vivado_pynq_proj_dir + "/finn-accel.bit"
        ...
        deploy_hwh_name = vivado_pynq_proj_dir + "/finn-accel.hwh"
