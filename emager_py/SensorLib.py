...
class HDSensor(object):
    ...
    def read(self, readtime, feedback=False, savetxt=False, savepath=None, window_bar=None, window_per=0):
        '''
        ...
        while (time.time() - start_time) < readtime:
            ...
            if data_packet is not None:
                ...
                pass
            if window_bar != None:
                percentage = (time.time()-start_time) / readtime
                window_bar.setValue(percentage * (window_per[1] - window_per[0]) + start_time)
        self.close()
    ...
    def sample(self):
        ...
        while True:
            ...
            if data_packet is not None:
                sample = [int.from_bytes(bytes([data_packet[0][i * 2], data_packet[0][i * 2 + 1]]), 'big', signed=True) for i in range(64)]
                ### ^ Iterating over byte pairs in line, 64 => n_channels, 2 bytes per ch.
...
if __name__ == '__main__':
    sensor = HDSensor('COM4', 1500000)
    # testdata = sensor.read(5, feedback=True)
    # sensor.open()
    sensor.read_full_buffer()